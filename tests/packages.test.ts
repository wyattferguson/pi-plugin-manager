...
describe("buildInstalledItems", () => {
  function buildItems(pkgs: Package[]): Array<{ label: string; description: string }> {
    return pkgs.map((p) => {
      const icon = p.hasUpdate ? "🔄" : p.type === "npm" ? "📦" : p.type === "git" ? "🔀" : "📁";
  ...
  test("npm without update", () => {
    const items = buildItems([makePkg({ name: "bar", type: "npm", hasUpdate: false })]);
